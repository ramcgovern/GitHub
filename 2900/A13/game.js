...
var LIGHT = {
	...
	//VARIABLES
	//These two arrays store the x and y positions of the light streams
	lightX : [],
	lightY : [],

	//FUNCTIONS
	//LIGHT.shoot()
	//When the bead reaches the top of the light stream it disappears
	...
	tick : function () {
		...
		let len = LIGHT.lightX.length; // number of keys
		...
		while ( i < len ) {
			...
			let x = LIGHT.lightX[ i ];
			let y = LIGHT.lightY[ i ];
			...
			if ( y >= LIGHT.TOP_ROW ) {
				...
				// PS.color( x, y, LIGHT.BG_COLOR ); *BM* No need to erase. The fader will do it!
				...
				if ( y >= LIGHT.TOP_ROW ) { // nope
					...
					PS.color( x, y, LIGHT.BG_COLOR ); // *BM* Set to background; the fader knows what color to start with
				}
...
PS.init = function ( system, options ) {
	...
	for ( let y = 0; y < LIGHT.BOTTOM_ROW; y += 1 ) {
		for ( let x = 0; x < LIGHT.GRID_WIDTH; x += 1 ) {
			...
			PS.fade( x, y, 20, { rgb : LIGHT.COLORS[ x ] } ); // *BM* This is where the magic happens!
		}
	}
	...
	//Bead Size
	// PS.scale( PS.ALL, 15, 90); // *BM* Use borders instead of scaling to avoid misfires
